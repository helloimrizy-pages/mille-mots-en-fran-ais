...
const RAW_IN = 'data/raw-frequency.json';
...
- example: { fr, en } — natural example sentence using the word
- synonyms: up to 2 common synonyms, each { word, note? }
- drop: true ONLY if the entry is a proper noun, a duplicate, a clitic contraction, or otherwise unsuitable
...
async function main() {
  const raw = await readJson<Array<{ rank: number; french: string }>>(RAW_IN);
  const existing = await readJson<Word[]>(OUT);
  const done = new Set(existing.map((w) => w.french));
  const todo = raw.filter((r) => !done.has(r.french));
  console.log(`Starting retry: ${existing.length} already enriched, ${todo.length} missing`);
  ...
  for (let i = 0; i < todo.length; i += BATCH_SIZE) {
    ...
    console.log(`Retry batch ${Math.floor(i / BATCH_SIZE) + 1} — ${first?.french}…${last?.french}`);
    ...
    try {
      res = await client.messages.create({
        ...
        messages: [{ role: 'user', content: `Enrich these ${batch.length} French words:\n${batch.map((b) => b.french).join('\n')}` }],
      });
    } catch (e) {
      console.warn(`  API error: ${(e as Error).message}`);
      continue;
    }
    ...
    if (!Array.isArray(parsed)) { console.warn('  expected array'); continue; }

    // Parse each item INDIVIDUALLY (key diff vs 02-enrich.ts)
    ...
    console.log(`  batch added ${addedThisBatch}, total: ${existing.length}`);
  }
